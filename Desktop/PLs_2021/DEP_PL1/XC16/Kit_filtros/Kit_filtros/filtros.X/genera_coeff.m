%SCRIPT de matlab realizado para generar archivos ".s", que serán usados
%para la implementación de filtros tipo FIR. Para ello, a través del
%archivo generado por la fdatool de MatLab se genera un filtro y se definen
%las características del mismo, así como también el tipo de ventana que se
%va a utilizar.

%Para este ejemplo en concreto, se diseña un filtro pasobanda tipo FIR con
%las siguientes características:

%Frecuencia de muestreo Fs= 10000 Hz
%FNyquist=Fs/2
%Orden del filtro N=256
%Frecuencias de corte del filtro Fc1= 490Hz y Fc2= 510Hz
%flag=scale - flag de muestreo
%Beta= 0,5 - parámetro de ventana
%Se usa una ventana Kaiser

close all
clear all

%Variable auxiliar igual al nombre del archivo generado por la fdatool
%Este script llama al archivo fpbanda2.m que contiene una función, que
...
%Ahora se construye el número iq
%Para ello se usa una función fi(v,s,w,f) donde se devuelve un objeto de punto fijo (fixed-point object)
...
%A partir de aquí, se establecen los paths para guardar el archivo .s
%generado, así como también los fprintf necesarios para que el aspecto del
...
path = 'C:\Users\evely\Desktop\PLs_2021\DEP_PL1\XC16\Kit_filtros\Kit_filtros\filtros.X\';
